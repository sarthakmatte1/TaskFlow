...
# TaskFlow — Docker + Kubernetes + Jenkins Deployment Guide
...
```
taskflow/                          ← your existing project root
├── backend/                       ← FastAPI app
...
```bash
cp .env.example .env
# Edit .env and set your own passwords and SECRET_KEY
nano .env
```
...
Edit `k8s/03-backend.yaml` and `k8s/04-frontend.yaml` — replace:
```
...
Edit `Jenkinsfile` line 18 — replace:
```
...
- Push a commit to `main` or `staging` branch
- Or manually: Open the job → Build Now
...
6. ✅ Ask for manual approval → Deploy to production (main branch)
...
| Image pull error | Check DockerHub credentials, image name spelling |
